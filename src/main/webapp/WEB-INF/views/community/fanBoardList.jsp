...
<!-- fireworks.js (폭죽 효과) -->
<script src="https://cdn.jsdelivr.net/npm/fireworks-js@2.1.2/dist/fireworks.js"></script>

<!-- Bootstrap 5 CSS (이미 포함되어 있으면 생략) -->
...
.vip-membership {
	...
	box-shadow: 0 4px 15px rgba(240, 147, 251, 0.5), 0 6px 20px
		rgba(245, 87, 108, 0.4);
	border-radius: 1rem;
...
@keyframes glow { 
0%{box-shadow: 0 0 10px rgba(240, 147, 251, 0.7);
}
100%{box-shadow:0 0 20px rgba(245,87,108,1);}
}
.text-truncate {
  white-space: nowrap;
  overflow: hidden;
...
.blind-card::after {
  ...
  font-weight: 700;
  z-index: 9999; /* 블러 영향을 피할 수 있도록 높은 z-index */
  text-align: center;
...
<div class="weverse-tabs d-flex justify-content-center">
  <ul class="nav nav-pills nav-fill">
	...
	<form id="artGroupInfo" name="artGroupInfo">
		<input type="hidden" id="artGroupNo" name="artGroupNo" value="${communityProfileVO.artGroupNo }"> <input
			type="hidden" id="memNo" name="memNo"
			value="${communityProfileVO.memNo }"> <input type="hidden"
			id="comProfileNo" name="comProfileNo"
			value="${communityProfileVO.comProfileNo }"> <input
			type="hidden" id="comNm" name="comNm"
			value="${communityProfileVO.comNm }"> <input type="hidden"
			id="saveFileLocate" name="saveFileLocate"
			value="${communityProfileVO.fileGroupVO.fileDetailVOList[0].fileSaveLocate }">
		<input type="hidden" id="userNo" name="userNo"
			value="${userVO.userNo }"> <input type="hidden" id="authNm"
			name="authNm" value="${userVO.userAuthList[0].authNm }">
	</form>
...
<!-- 생일 알림 로직 -->
<!-- Fireworks.js 최신 CDN (v2) -->
<%-- ${artistBirth } --%>
	<c:if test="${ not empty artistBirth && artistBirth != '없음' }">
	...
	<div class="container mt-4 mx-auto">
					...
					<c:forEach var="artistVO" items="${artistGroupVO.artistVOList }">
						<c:if test="${reply.memNo == artistVO.memNo && reply.replyDelyn=='N'}">
							<div class="swiper-slide">
								<div
									class="artist-card text-white text-center d-flex flex-column justify-content-end p-3"
									...
									<div class="artist-text position-relative">
										<p class="fw-bold mb-1">${reply.comNm}<span
												class="badge-star">🎵</span>
										</p>
										<p class="artist-message" style="cursor: pointer;"
											onclick="fn_load_replies(${reply.boardNo})">${reply.replyContent}</p>
									</div>
								</div>
							</div>
						</c:if>
		...
		<div class="modal fade" id="replyModal" tabindex="-1"
			aria-labelledby="replyModalLabel" aria-hidden="false">
				...
				<div class="modal-content">
					<div class="modal-header">
						...
						<button type="button" class="btn-close" data-bs-dismiss="modal"
							id="modal_close" onclick="fn_reply_modal_close()"
		...
		<div class="container mt-4">
			<div class="col-md-8 mx-auto">
				<c:choose>
					<c:when
						test="${(communityProfileVO.comDelyn=='N')&&(userVO.userNo == communityProfileVO.memNo) && (userVO.userAuthList[0].authNm=='ROLE_MEM') && ((communityProfileVO.comProfileNo!=0) && (communityProfileVO.comProfileNo!=null))}">
						<button class="btn-weverse" data-bs-toggle="modal"
							data-bs-target="#postModal">
								<spring:message code="commu.post.write" />
							</button>
					</c:when>
					<c:otherwise>
						<button class="btn-weverse" data-bs-toggle="modal" data-bs-target="#postModal" disabled>회원만 작성 가능합니다</button>
					</c:otherwise>
				</c:choose>
			</div>
		</div>
	...
	<!-- 모달 -->
	<div class="modal fade custom-modal" id="postModal" tabindex="-1"
		aria-labelledby="postModalLabel" aria-hidden="false">
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<form name="frm" action="/oho/community/addBoard" method="post"
					...
					<div class="modal-footer custom-footer">
						<div>
							<label for="formBoardFile" class="form-label"></label> <input
								class="form-control" type="file" id="formBoardFile" multiple
								onchange="readBoardFile(this)" name="uploadFile">
								<div id="editBoardImg"></div>
						</div>
						
						<div>
							<input type="checkbox" name="boardOnlyFan" value="Y" />Hide from
							Artists
						</div>
	...
	<!-- 게시글 수정 모달 -->
	<div class="modal fade custom-modal" id="postEditModal" tabindex="-1"
		aria-labelledby="postModalLabel" aria-hidden="false">
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				<!-- 모달 헤더 -->
	            ...
	            <form id="boardEditForm" name="boardEditForm" action="/oho/community/editBoard" method="post" enctype="multipart/form-data">
					...
					<div class="modal-body">
						<input type="text" class="form-control mb-1" id="boardEditTitle" name="boardTitle"
						  placeholder="제목을 입력해주세요" maxlength="30" oninput="checkLength(this, 30, 'titleCounter')" />
						<small><span id="titleCounter">0</span>/30</small>
					...
					<div class="modal-footer custom-footer">
						...
						<button type="button" class="btn btn-custom" id="submitBtn"
							onclick="fn_board_edit_submit()">등록</button>
					</div>
				</form>
	...
	<c:if
		test="${(communityProfileVO.comDelyn == 'Y')||(userVO.userNo == communityProfileVO.memNo) && (userVO.userAuthList[0].authNm=='ROLE_MEM') && (communityProfileVO.comProfileNo==0)}">
		<!-- 비회원용 UI -->
		<div class="container text-center mt-4">
			<div
				class="alert alert-dark d-flex flex-column align-items-center p-4">
				<h5 class="fw-bold">커뮤니티에 가입해 더 많은 기능을 사용해보세요!</h5>

				<a
					href="/oho/community/join?memNo=${communityProfileVO.memNo }&artGroupNo=${communityProfileVO.artGroupNo}"
					class="btn btn-success rounded-pill px-4">커뮤니티 가입</a>
			</div>
		</div>
	</c:if>
	...
	<div class="container mt-4">
		<div class="row gx-1">
			...
			<!-- 왼쪽: 피드 게시물 -->
			<div class="col-md-8" id="divBoards"></div>

			<!-- 오른쪽: 사이드바 -->
			<div class="col-md-4">
				<a
					href="/oho/groupProfile?artGroupNo=${communityProfileVO.artGroupNo }"
					style="text-decoration: none;">
					<div class="tempest-box mb-3">
						<img
							src="/upload${artistGroupVO.fileGroupVO.fileDetailVOList[0].fileSaveLocate }"
							class="img-fluid rounded">
						<h5 class="mt-2 text-white py-2 px-3 rounded"
							style="font-size: 1.75rem;">${artistGroupVO.artGroupNm }</h5>
					</div>
				</a>
				<c:choose>
					<c:when test="${membershipYn == 'Y'}">
						<div
							class="fan-card text-center mb-3 p-3 text-white vip-membership">
							<strong>${artistGroupVO.artGroupNm} Digital Membership</strong> <span
								class="badge rounded-pill bg-warning text-dark ms-2">멤버십
								회원</span>
						</div>
					</c:when>
					<c:otherwise>
						...
						<form action="${memberShipLink }" method="${memberShipMethod }" target="_blank">
				...
				<!-- 오세인 프로필 카드 -->
				<c:choose>
					<c:when test="${communityProfileVO.comProfileNo != 0 && communityProfileVO.comDelyn=='N'}">
						<a
							href="/oho/community/profileDetail?comProfileNo=${communityProfileVO.comProfileNo}&from=fanBoardList"
							class="no-underline">
							<div class="fan-card profile-card text-center mb-3">
								...
								<code>${communityProfileVO.comNm}<c:if
										test="${communityProfileVO.comAuth=='ROLE_ART' }">
										<span class="badge-star">🎵</span>
									</c:if>
									<c:if test="${membershipYn == 'Y'}">
										<span class="badge bg-primary texet-white ms-1 align-middle"
											title="멤버십 회원"><i class="fa-solid fa-gem"></i></span>
									</c:if>
								</code>
								<br> <small class="text-muted">Joined<br>
					...
					<c:otherwise>
						<a
							href="/oho/community/join?memNo=${userVO.userNo}&artGroupNo=${communityProfileVO.artGroupNo}"
				...
				<div class="notice-box mb-4">
				  <div class="card shadow-sm rounded-4 px-3 py-3 bg-light border-0">
				    <div class="d-flex justify-content-between align-items-center mb-2">
				      <div class="fw-bold text-dark" target="_blank">
				        <i class="bi bi-megaphone-fill text-warning me-1"></i>
				        <spring:message code="commu.notice" />
				      </div>
				      <a href="/oho/community/notice?artGroupNo=${artistGroupVO.artGroupNo}"
				         class="text-primary small fw-semibold" target="_blank"><spring:message code="commu.more"/></a>
				    </div>
	...
	<div id="endMessage"
		style="display: none; text-align: center; font-size: 16px; color: black;">
...
console.log("myComProfileNo===================",myComProfileNo);
	let currentPage =1;
		...
		console.log("data : ",data);
		 $.ajax({
	       ...
	       , success: function(communityReplyVO) {
			...
			error : function() {
	            alert('입력한 댓글이 너무 깁니다 다시 입력해주세요');
	           	     ...
	           	     str+=`	  <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
	           	     			<li>
	                   				<a class="dropdown-item d-flex align-items-center aReport" href="#" data-bs-toggle="modal"
	                   					data-bs-target="#reportModal" data-report-gubun="게시글" data-report-board-no="\${communityReplyVO.replyNo}"
...
/* 댓글 좋아요 버튼, 게시글 좋아요 버튼 누르는 펑션 만듬 ( 처리해야함 )
    ...
    */
    $.each(boardListVO, function(i, element) {
    	...
    	console.log("권한 체크 : ",element.boardOnlyFan,authNm);
		if(element.boardDelyn == 'N' && element.comAuth=='ROLE_MEM'){
        str += `
			
        <div>
            <div class="card shadow-sm rounded `;
            if((myComProfileNo==0 || authNm == 'ROLE_ART') &&element.boardOnlyFan == 'Y' ){
            	str+=`blind-card`;
            }
       ...
       str+=`     <div style="display: flex; gap: 20px;">
                    <a href="/oho/community/profileDetail?comProfileNo=\${element.comProfileNo}" class="text-dark text-decoration-none fw-bold"><h5 class="card-title fw-bold"><strong>\${element.comNm}`;
                
                     if(element.communityProfileVO.membershipYn=='Y'){
                    	 str+=`<span class="badge bg-primary texet-white ms-1 align-middle"
     						title="멤버십 회원"><i class="fa-solid fa-gem"></i></span>`;
                     }
        ...
        if(element.memNo==userNo){
        	...
        	<div class="position-absolute top-1 end-0">
        		...
        		<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        			<li>
            		<a class="dropdown-item d-flex align-items-center aReport" href="#" data-bs-toggle="modal"
    					data-bs-target="#reportModal" data-report-board-no="\${element.boardNo}" data-report-gubun="게시글"
                    ...
                    if (!element.replyList || element.replyList.length <= 0) {
                        ...
                        element.replyList.forEach(reply => {
                            ...
                            if(reply.comAuth=='ROLE_ART'){
                              ...
                              str+=` 	                  	                              
                              		<p id="repCreateDt">\${reply.repCreateDate}</p>
...
const swiper = new Swiper('.mySwiper', {
	...
	setTimeout(function() {
	    document.getElementById('giftBox').style.display = 'none';  // 5초 후 GIF 숨기기
	}, 5000); // 5000ms = 5초
