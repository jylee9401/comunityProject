...
function fn_search(page) {
	...
	const datas = {
		...
		unitPricePre: $('#unitPricePre').val(),
		unitPricePost: $('#unitPricePost').val(),
	...
	axios.post('/admin/shop/goodsListSearchPost', datas).then(resp => {
...
function renderTable(data, currentPage) {
  ...
  data.forEach( (item, idx) => {
	...
	//item.goodsVOList.add("artGroupNm" , ${item.artGroupNm});
	let artGroupNm = item.artGroupNm;
	
	item.goodsVOList.forEach( (goodsItem, idx)  => {
		
		if(goodsItem.gdsType == 'G') {
			goodsItem.gdsType = 'Group';
		}
		
		else if(goodsItem.gdsType == 'A') {
			goodsItem.gdsType = 'Album';
		}
				
		else if(goodsItem.gdsType == 'M') {
			goodsItem.gdsType = 'MemberShip';
		}
		
		row += `<tr onclick="location.href='adGoodsDetail?gdsNo=${goodsItem.gdsNo}'" style="cursor: pointer;">
					  <td>${goodsItem.rrnum}</td>
					  <td style="text-align:left;">${artGroupNm}</td>
					  <td style="text-align:center;">${goodsItem.gdsType}</td>
					  <td style="text-align:left;">${goodsItem.gdsNm}</td>
					  <td style="text-align:right;">${goodsItem.unitPrice.toLocaleString()}</td>
					  <td style="text-align:center;">${goodsItem.pic}</td>
					  <td style="text-align:center;">${goodsItem.regDt}</td>
					  <td style="text-align:center;">${goodsItem.gdsDelYn}</td>
				</tr>
			   `
	})
  });
