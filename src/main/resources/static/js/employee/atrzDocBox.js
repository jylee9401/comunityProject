...
$(document).ready(function() {
	...
	$('#approval-tab').on('click', function() {
		changeTab(this);
	});
...
function fn_search(page) {
	...
	axios.post('/emp/atrzDocBoxAjax', datas).then(resp => {
	      ...
	      dept.employeeVOList?.forEach(emp => {
	        emp.atrzDocVOList?.forEach(doc => {
	          ...
	          tr.innerHTML = `
	            ...
	            <td>${doc.atrzDocNo}</td>
	            <td>${renderStatusBadge(doc.atrzSttsCd)}</td>
	          `;
...
function renderStatusBadge(status){
	if(status === '진행중'){
		...
		return '<span class="badge bg-danger">반려</span>';
	}else{
		return '<span class="badge bg-secondary">완료</span>';
	}
}
