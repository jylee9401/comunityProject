...
function fn_search(page) {
	...
	const datas = {
		boardOnlyMembership: $('#boardOnlyMembership').val(),
		boardOnlyFan: $('#boardOnlyFan').val(),
		boardDelyn: $('#boardDelyn').val(),
		urlCategory: $('#urlCategory').val(),
		memNo: $('#memNo').val(),
		boardCreateDate: $('#boardCreateDate input').val(),
	...
	axios.post('/admin/community/postListAjax', datas).then(resp => {
...
function renderTable(data, currentPage) {
  ...
  data.forEach((item, idx) => {
	...
	console.log("boardCreateDt2===",item);
	
	const displayToggle = item.boardDelyn === 'Y'  ? 'secondary' : 'danger';
	const buttonToggle = item.boardDelyn=== 'Y'  ? '재등록' : '숨김';
	const functionToggle = item.boardDelyn=== 'Y'
	  ? `confirmUnDelete(${item.boardNo})`
	  : `confirmDelete(${item.boardNo})`;
...
/*	  <td style="text-align:left;">
	    <div class="content-preview-wrapper">
	      ...
	      <button type="button" class="btn btn-link btn-sm toggle-btn">더보기</button>
	    </div>
	  </td>*/
    const row = `
      <tr onclick="location.href='postDetail?boardNo=${item.boardNo}'" style="cursor: pointer;">
        <td>${startNo+idx+1}</td>
        <td style="text-align:left;">${item.boardTitle}</td>
		<!-- 테이블 셀 내부 -->

        <td>${item.boardOnlyMembership}</td>
        <td>${item.boardOnlyFan}</td>
        <td>${item.boardDelyn}</td>
        <td style="white-space: normal; min-width: 100px;">${item.boardCreateDt2}</td>
        <td style="white-space: normal; min-width: 100px;">${item.urlCategory}</td>
        <td>${item.memNo}</td>
		<td>
		  <button class="btn btn-${displayToggle} btn-sm"
		          onclick="event.stopPropagation(); ${functionToggle}">
		    ${buttonToggle}
		  </button>
		</td>
      </tr>
...
$('#boardCreateDate').datetimepicker({
	format: 'YYYY-MM-DD'
});
