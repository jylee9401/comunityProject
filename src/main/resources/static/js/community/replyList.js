...
function fn_search(page) {
	...
	const datas = {
		...
		replpyDelyn: $('#replyDelyn').val(),
		replyCreateDate: $('#replyCreateDate input').val(),
		memNo: $('#memNo').val(),
		boardNo: $('#boardNo').val(),
		mediaPostNo: $('#mediaPostNo').val(),
		tkNo: $('#tkNo').val(),
		replyNo: $('#replyNo').val(),
	...
	axios.post('/admin/community/replyListAjax', datas).then(resp => {
...
function renderTable(data, currentPage) {
  ...
  data.forEach((item, idx) => {
	const displayToggle = item.replyDelyn === 'Y'  ? 'secondary' : 'danger';
	const buttonToggle = item.replyDelyn=== 'Y'  ? '재등록' : '숨김';
	const functionToggle = item.replyDelyn=== 'Y'
	  ? `confirmUnDelete(${item.replyNo})`
	  : `confirmDelete(${item.replyNo})`;
    const row = `
      <tr>
        <td>${startNo+idx+1}</td>

		<td style="text-align:left;">
		  <div style="white-space: normal; min-width: 150px; word-break: break-word;">
		    <div class="reply-content collapsed">
		      ${item.replyContent}
		    </div>
		    ${item.replyContent.length > 100
		      ? `<button type="button" class="btn btn-link btn-sm toggle-reply">더보기</button>`
		      : ''}
		  </div>
		</td>
        <td style="white-space: normal; min-width: 100px;">${item.urlCategory}</td>
        <td style="white-space: normal; min-width: 100px;">${item.replyCreateDt2}</td>
        <td>${item.replyDelyn}</td>
        <td>${item.memNo}</td>
        <td>${item.boardNo}</td>
        <td>${item.mediaPostNo}</td>
        <td>${item.tkNo}</td>
        <td>${item.replyNo}</td>
...
$('#replyCreateDate').datetimepicker({
	format: 'YYYY-MM-DD'
});
